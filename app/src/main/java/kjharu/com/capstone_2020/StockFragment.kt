        ...
        lv.onItemClickListener = AdapterView.OnItemClickListener { parent, view, position, id ->
            databaseUser.addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(p0: DataSnapshot) {
                    ...
                    //현재 수정하려는 재고의 값들
                    //현재 재고명
                    var nameNow = p0.child(userId.toString()).child(datkey).child("stock")
                        .child("s" + position).child("stockName").value
                    //현재 재고량
                    var numNow = p0.child(userId.toString()).child(datkey).child("stock")
                        .child("s" + position).child("stockNum").value
                    //현재 재고원가
                    var priceNow = p0.child(userId.toString()).child(datkey).child("stock")
                        .child("s" + position).child("stockPrice").value
                    ...
                    builder.setPositiveButton("확인") { dialogInterface, i ->
                        ...
                        //DB에 저장
                        databaseUser.child(userId.toString()).child(datkey).child("stock")
                            .child("s" + position).child("stockName")
                            .setValue(stockName.text.toString())
                        databaseUser.child(userId.toString()).child(datkey).child("stock")
                            .child("s" + position).child("stockNum")
                            .setValue(stockNum.text.toString())
                        databaseUser.child(userId.toString()).child(datkey).child("stock")
                            .child("s" + position).child("stockPrice")
                            .setValue(stockPrice.text.toString())
                    }
                    adapter.notifyDataSetChanged()
                    adapter.notifyDataSetInvalidated()
            ...
            databaseUser.addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(p0: DataSnapshot) {
                    ...
                    builder.setPositiveButton("확인") { dialogInterface, i ->
                        ...
                        val databaseStockForGet =
                            p0.child(userId.toString()).child(datkey).child("stock")
                        //하나씩 앞당겨주기
                        for (i in position..databaseStockForGet.childrenCount - 1) {
                            //다음 이름 가져오기
                            var nextName = databaseStockForGet.child("s" + (i + 1))
                                .child("stockName").value.toString()
                            //앞당겨저장
                            databaseStock.child("s" + i).child("stockName").setValue(nextName)
                            //다음 재고량 가져오기
                            var nextNum = databaseStockForGet.child("s" + (i + 1))
                                .child("stockNum").value.toString()
                            //앞당겨저장
                        ...
                        //맨 마지막꺼 없애기(앞당겨줬으니)
                        databaseUser.child(userId.toString()).child(datkey).child("stock")
                            .child("s" + (databaseStockForGet.childrenCount - 1)).removeValue()
                    }
                    adapter.notifyDataSetChanged()
                    adapter.notifyDataSetInvalidated()
            ...
            databaseUser.addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(p0: DataSnapshot) {
                    ...
                    builder.setPositiveButton("확인") { dialogInterface, i ->
                        ...
                        //DB에 저장
                        databaseUser.child(userId.toString()).child(datkey).child("stock")
                            .child("s" + count).child("stockName")
                            .setValue(stockName.text.toString())
                        databaseUser.child(userId.toString()).child(datkey).child("stock")
                            .child("s" + count).child("stockNum").setValue(stockNum.text.toString())
                        databaseUser.child(userId.toString()).child(datkey).child("stock")
                            .child("s" + count).child("stockPrice")
                            .setValue(stockPrice.text.toString())
                    }
                    adapter.notifyDataSetChanged()
                    adapter.notifyDataSetInvalidated()
